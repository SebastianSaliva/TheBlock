...
class CustomMode1: public FractalMode {
    ...
    void draw(float x, float y, float l, int n, string i) {
        ...
        if (i == "begin") {
        draw(x-(l/2) + 1, y, l/1.5, n-1, "left");
        draw(x+(l/2) + 1, y, l/1.5, n-1, "right");
        draw(x, y-(l/2)+1, l/1.5, n-1, "up");
        draw(x, y+(l/2)+1, l/1.5, n-1, "down");
        }

        if (!(i == "left")) {draw(x+(l/2) + 1, y, l/1.5, n-1, "right");}
        if (!(i== "right")) {draw(x-(l/2) + 1, y, l/1.5, n-1, "left");}
        if (!(i== "up")) {draw(x, y+(l/2)+1, l/1.5, n-1, "down");}
        if (!(i== "down")) {draw(x, y-(l/2)+1, l/1.5, n-1, "up");}
